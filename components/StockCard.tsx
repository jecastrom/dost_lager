...
import { Plus, Minus, Hash, Info, PackagePlus, MapPin } from 'lucide-react';
...
export const StockCard: React.FC<StockCardProps> = ({ item, onUpdate, onAddStock, viewMode, theme }) => {
  const [bulkInput, setBulkInput] = useState('');
  const isDark = theme === 'dark';

  const handleAdjust = (amount: number) => {
    onUpdate(item.id, Math.max(0, item.stockLevel + amount));
  };

  const handleBulkAdjust = (sign: number) => {
  ...
  if (viewMode === 'list') {
    return (
      <div className={`flex flex-col sm:flex-row sm:items-center gap-3 md:gap-4 p-3 border rounded-lg transition-all group backdrop-blur-sm ${
        isDark 
          ? 'bg-slate-800/50 border-slate-700/50 hover:border-slate-600' 
          : 'bg-white/70 border-[#CACCCE]/50 hover:border-[#0077B5] shadow-sm'
      }`}>
        {/* Color Indicator (Hidden on mobile to save space, or adjusted) */}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1 sm:mb-0">
            <h3 className={`text-sm font-semibold truncate transition-colors ${isDark ? 'text-slate-100' : 'text-[#313335]'}`}>
              {item.name}
            </h3>
  ...
  return (
    <div className={`border rounded-xl p-4 flex flex-col gap-4 transition-all duration-300 group backdrop-blur-md ${
      isDark 
      ...
      <div className="flex justify-between items-start gap-2">
        ...
        <button onClick={onAddStock} className="text-[#86888A] hover:text-[#0077B5] transition-colors">
           <PackagePlus size={18} />
        </button>
      </div>
      ...
      {/* Controls */}
      <div className={`mt-auto pt-4 border-t transition-colors ${isDark ? 'border-slate-700/50' : 'border-[#CACCCE]/40'}`}>
        <div className="flex items-center justify-between gap-2">
          {/* Quick Actions - Grayscale in Light Mode */}
          ...
          {/* Bulk Action - Spanish Blue Branding */}
