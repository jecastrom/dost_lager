      ...
      {activeSection === 'audit' && (
        <div className="space-y-4 md:space-y-6 animate-in slide-in-from-bottom-4 duration-300">
          ...
          <DocCard title={t('Ablauf — Schritt für Schritt', 'Flow — Step by Step')} icon={<ArrowRight size={20} />}>
            <div className="space-y-3 text-xs">
              <div><strong>{t('1. Setup — 3-Schritt-Assistent', '1. Setup — 3-Step Wizard')}</strong>
                <div className="space-y-1.5 mt-1.5 ml-2">
                  <p><span className={`inline-block w-4 h-4 rounded-full text-center text-[10px] leading-4 font-bold mr-1.5 ${isDark ? 'bg-[#0077B5]/20 text-[#0077B5]' : 'bg-[#0077B5]/10 text-[#0077B5]'}`}>1</span>{t('Wo zählen? — Lagerort-Grid mit Artikelanzahl und letztem Inventurdatum pro Standort.', 'Where to count? — Warehouse grid with item count and last audit date per location.')}</p>
                  <p><span className={`inline-block w-4 h-4 rounded-full text-center text-[10px] leading-4 font-bold mr-1.5 ${isDark ? 'bg-[#0077B5]/20 text-[#0077B5]' : 'bg-[#0077B5]/10 text-[#0077B5]'}`}>2</span>{t('Bezeichnung — Automatisch generiert (z.B. "Inventur – Lager 3 – 05.03.2026"), editierbar.', 'Name — Auto-generated (e.g. "Inventur – Lager 3 – 05.03.2026"), editable.')}</p>
                  <p><span className={`inline-block w-4 h-4 rounded-full text-center text-[10px] leading-4 font-bold mr-1.5 ${isDark ? 'bg-[#0077B5]/20 text-[#0077B5]' : 'bg-[#0077B5]/10 text-[#0077B5]'}`}>3</span>{t('Art der Zählung — Schnellzählung (grün) oder Normale Inventur (blau). Per-Audit Blindmodus-Toggle (versteckt bei globalem Blind).', 'Audit type — Quick count (green) or Normal audit (blue). Per-audit blind toggle (hidden when global blind is on).')}</p>
                </div>
              </div>
          ...
          <DocCard title={t('Warenkorb-UX — Premium-Feeling', 'Cart UX — Premium Feel')} icon={<Package size={20} />}>
            <div className="space-y-2 text-xs">
              ...
              <p><strong>{t('Sticky Header', 'Sticky Header')}:</strong> {t('Fixierter Kontext-Balken: Inventur-Name, Standort, Modus-Badge, Fortschritt "X/Y" mit Verlaufsbalken. Backdrop-Blur.', 'Fixed context bar: audit name, location, mode badge, progress "X/Y" with gradient bar. Backdrop blur.')}</p>
              <p><strong>{t('Bodenleiste', 'Bottom Bar')}:</strong> {t('Fixiert über Mobile-Nav. Artikelanzahl + Stück. "Abbrechen" + "Fertig →". Safe-area-inset + Sidebar-Offset.', 'Fixed above mobile nav. Item count + pieces. "Cancel" + "Done →". Safe-area-inset + sidebar offset.')}</p>
              <p><strong>{t('Auto-Speicherung', 'Auto-Save')}:</strong> {t('Aktive Zählung wird alle 500ms als Draft in IndexedDB gesichert. Überlebt Seitenneuladung. Wird bei Abschluss/Abbruch gelöscht.', 'Active count is saved as draft to IndexedDB every 500ms. Survives page reload. Cleared on finish/cancel.')}</p>
            </div>
          </DocCard>
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className={`rounded-xl border p-4 ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3"><span className="shrink-0 flex items-center gap-0.5"><WifiOff size={13} className="text-orange-500" /><Database size={11} className="text-orange-500" /></span><span className={`text-[11px] px-2 py-0.5 rounded font-bold ${isDark ? 'bg-orange-500/10 text-orange-400' : 'bg-orange-50 text-orange-600'}`}>{t('Offline', 'Offline')}</span><span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>— {t('Kein Netz — Daten lokal in IndexedDB gesichert. Zählung läuft normal.', 'No network — data saved locally in IndexedDB. Counting continues normally.')}</span></div>
                  <div className="flex items-center gap-3"><Database size={13} className="text-amber-500 animate-pulse shrink-0" /><span className={`text-[11px] px-2 py-0.5 rounded font-bold ${isDark ? 'bg-amber-500/10 text-amber-400' : 'bg-amber-50 text-amber-600'}`}>{t('Synchronisiere', 'Syncing')}</span><span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>— {t('Warteschlange wird an den Server gesendet.', 'Queue is being sent to the server.')}</span></div>
                </div>
              </div>
            </div>
          </DocCard>
          ...
          <DocCard title={t('Schicht 4: Sync-Indikator', 'Layer 4: Sync Indicator')} icon={<Wifi size={20} />}>
            <div className="space-y-2 text-xs">
              <div className="space-y-1.5 ml-2">
                ...
                <div className="flex items-start gap-2"><span className="flex items-center gap-0.5"><WifiOff size={12} className="text-orange-500" /><Database size={10} className="text-orange-500" /></span> <span><strong className="text-orange-500">{t('Offline', 'Offline')}</strong> — <span className={`text-xs font-normal ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('Kein Netz. Daten lokal in IndexedDB gesichert.', 'No network. Data saved locally in IndexedDB.')}</span></span></div>
                <div className="flex items-start gap-2"><Database size={12} className="text-amber-500 animate-pulse shrink-0 mt-0.5" /> <span><strong className="text-amber-500">{t('Synchronisiere…', 'Syncing…')}</strong> — <span className={`text-xs font-normal ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('Warteschlange wird an den Server gesendet.', 'Queue is being sent to the server.')}</span></span></div>
              </div>
            </div>
          </DocCard>
        </div>
