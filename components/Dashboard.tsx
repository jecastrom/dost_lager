...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  // --- Recent Activity (Last 10 entries from all sources) ---
  type ActivityEntry = {
    id: string;
    timestamp: number;
    type: 'stock' | 'order' | 'receipt' | 'ticket';
    icon: 'add' | 'remove' | 'order' | 'receipt' | 'ticket';
    title: string;
    detail: string;
    context?: string;
  };
  ...
  const recentActivity = useMemo(() => {
    ...
    logs.forEach(log => {
      entries.push({
        ...
        type: 'stock',
        icon: log.action,
        title: `${log.action === 'add' ? '+' : '-'}${log.quantity} ${log.itemName}`,
        detail: log.source || 'Manuell',
        context: log.context,
      });
    });

    // Orders (from PurchaseOrder[]) — use createdAt or fallback
    orders.forEach(order => {
      const ts = order.createdAt || order.lastUpdated || 0;
      if (ts > 0) {
        entries.push({
          id: `order-${order.id}`,
          timestamp: ts,
          ...
          detail: order.supplier || 'Unbekannt',
          context: order.status,
        });
      }
    });

    // Receipts (from ReceiptMaster[]) — use createdAt or fallback
    receipts.forEach(r => {
      const ts = r.createdAt || r.lastUpdated || 0;
      if (ts > 0) {
        entries.push({
          id: `receipt-${r.poId}`,
          timestamp: ts,
    ...
    return entries
      ...
      .slice(0, 10);
  }, [logs, orders, receipts, tickets]);

  // Keep legacy recentLogs for backward compat with existing render code
      ...
      <div className="space-y-3">
        ...
        <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          ...
          <div className="md:hidden divide-y divide-slate-500/10">
            {recentActivity.length === 0 && recentLogs.length === 0 ? (
              <div className="p-8 text-center text-slate-500 italic">Keine Aktivitäten verzeichnet.</div>
            ) : recentActivity.length > 0 ? (
              recentActivity.map(entry => {
                const iconColor = entry.icon === 'add' ? (isDark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-50 text-emerald-600')
                  : entry.icon === 'remove' ? (isDark ? 'bg-red-500/15 text-red-400' : 'bg-red-50 text-red-600')
                    : entry.icon === 'order' ? (isDark ? 'bg-blue-500/15 text-blue-400' : 'bg-blue-50 text-blue-600')
                      : entry.icon === 'receipt' ? (isDark ? 'bg-amber-500/15 text-amber-400' : 'bg-amber-50 text-amber-600')
                        : (isDark ? 'bg-orange-500/15 text-orange-400' : 'bg-orange-50 text-orange-600');
                const IconEl = entry.icon === 'order' ? FileText : entry.icon === 'receipt' ? Truck : entry.icon === 'ticket' ? AlertTriangle : Package;
                return (
                  <div key={entry.id} className={`p-3.5 ${isDark ? 'active:bg-slate-800/50' : 'active:bg-slate-50'}`}>
                    <div className="flex items-start gap-3">
                      <div className={`shrink-0 w-11 h-11 rounded-xl flex items-center justify-center ${iconColor}`}>
                        <IconEl size={18} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className={`font-bold text-sm truncate ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{entry.title}</div>
