...
import { ArrowLeft, Calendar, User, Activity, FileText, Briefcase, MousePointer2, ClipboardCheck, AlertOctagon } from 'lucide-react';
...
export const StockLogView: React.FC<StockLogViewProps> = ({ logs, onBack, theme }) => {
    ...
    const [activeFilter, setActiveFilter] = useState<'all' | 'manual' | 'po-normal' | 'po-project' | 'audit'>('all');
    ...
    const filteredLogs = sortedLogs.filter(log => {
        ...
        if (activeFilter === 'audit') return log.context === 'audit-quick' || log.context === 'audit-normal';
        return true;
    });
        ...
        <div className="max-w-[1600px] mx-auto pb-20 animate-in fade-in slide-in-from-right-8 duration-300">
            ...
            <div className={`flex gap-6 border-b px-2 mb-6 overflow-x-auto ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
                <button
                    ...
                    className={`pb-3 font-bold text-sm transition-all border-b-2 whitespace-nowrap ${activeFilter === 'all'
                ...
                <button
                    ...
                    className={`pb-3 font-bold text-sm flex items-center gap-2 transition-all border-b-2 whitespace-nowrap ${activeFilter === 'manual'
                            ? 'text-[#0077B5] border-[#0077B5]'
                            : 'text-slate-500 border-transparent hover:text-slate-700 dark:hover:text-slate-300'
                        }`}
                >
                ...
                <button
                    ...
                    className={`pb-3 font-bold text-sm transition-all border-b-2 whitespace-nowrap ${activeFilter === 'po-normal'
                            ? 'text-[#0077B5] border-[#0077B5]'
                            : 'text-slate-500 border-transparent hover:text-slate-700 dark:hover:text-slate-300'
                        }`}
                >
                ...
                <button
                    ...
                    className={`pb-3 font-bold text-sm flex items-center gap-2 transition-all border-b-2 whitespace-nowrap ${activeFilter === 'po-project'
                            ? 'text-[#0077B5] border-[#0077B5]'
                            : 'text-slate-500 border-transparent hover:text-slate-700 dark:hover:text-slate-300'
                        }`}
                >
                ...
                <button
                    ...
                    className={`pb-3 font-bold text-sm flex items-center gap-2 transition-all border-b-2 whitespace-nowrap ${activeFilter === 'audit'
                            ? 'text-[#0077B5] border-[#0077B5]'
                            : 'text-slate-500 border-transparent hover:text-slate-700 dark:hover:text-slate-300'
                        }`}
                >
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {/* Audit metadata */}
                                        {(log.context === 'audit-quick' || log.context === 'audit-normal') && log.approvedByName && (
                                            <div className="flex items-center justify-between">
                                                <span className="text-xs font-bold uppercase text-slate-500">
                                                    {log.context === 'audit-normal' ? 'Genehmigt' : 'Gezählt'}
                                                </span>
                                                <span className={`text-xs font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                                                    {log.context === 'audit-normal' ? log.approvedByName : log.countedByName}
                                                </span>
                                            </div>
                                        )}
                                        ...
                                        >
                                            ...
                                            <td className="p-4">
                                                <div className="flex flex-col gap-1.5 items-start">
                                                    ...
                                                    {(log.context === 'audit-quick' || log.context === 'audit-normal') && (
                                                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${isDark ? 'bg-purple-500/10 border-purple-500/20 text-purple-400' : 'bg-purple-50 border-purple-200 text-purple-600'}`}>
                                                            <ClipboardCheck size={10} /> {log.context === 'audit-quick' ? 'SCHNELL' : 'INVENTUR'}
                                                        </span>
                                                    )}

                                                    {log.action === 'write-off' && (
                                                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${isDark ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-red-50 border-red-200 text-red-600'}`}>
                                                            <AlertOctagon size={10} /> ABSCHREIBUNG
                                                        </span>
                                                    )}
                                                </div>
                                            </td>
