...
const OrderStatusBadges = ({ order, linkedReceipt, theme }: { order: PurchaseOrder, linkedReceipt?: ReceiptMaster, theme: Theme }) => {
    ...
    if (order.isArchived) {
        ...
        badges.push(
            <span key="life-force-closed" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${isDark ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-emerald-50 text-emerald-600 border-emerald-200'
                }`}>
                Erledigt
            </span>
        );
        ...
        badges.push(
            <span key="life-open" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${isDark ? 'text-slate-400 border-slate-700' : 'text-slate-600 border-slate-300 bg-white'
                }`}>
                Offen
            </span>
        );
        ...
        if (!hasQualityStatus) {
            badges.push(
                <span key="life-partial" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${isDark ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-amber-50 text-amber-600 border-amber-200'
                    }`}>
                    Teillieferung
                </span>
            );
        ...
        badges.push(
            <span key="life-done" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${isDark ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-emerald-50 text-emerald-600 border-emerald-200'
                }`}>
                Erledigt
            </span>
        );
        ...
        badges.push(
            <span key="life-over" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${isDark ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-orange-50 text-orange-600 border-orange-200'
                }`}>
                Übermenge
            </span>
        );
    ...
    if (linkedReceipt) {
        ...
        if (s === 'Wartet auf Lieferung') {
            ...
            badges.push(
                <span key="proc-damage" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider flex items-center gap-1 ${isDark ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-red-50 text-red-600 border-red-200'
                    }`}>
            ...
            badges.push(
                <span key="proc-wrong" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider flex items-center gap-1 ${isDark ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-orange-50 text-orange-600 border-orange-200'
                    }`}>
            ...
            badges.push(
                <span key="proc-both" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider flex items-center gap-1 ${isDark ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-red-50 text-red-600 border-red-200'
                    }`}>
            ...
            if (totalReceived <= totalOrdered) {
                badges.push(
                    <span key="proc-over" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider flex items-center gap-1 ${isDark ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-orange-50 text-orange-600 border-orange-200'
                        }`}>
            ...
            badges.push(
                <span key="proc-rejected" className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider flex items-center gap-1 ${isDark ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-red-50 text-red-600 border-red-200'
                    }`}>
...
export const OrderManagement: React.FC<OrderManagementProps> = ({
    ...
    const MenuItem = ({ icon: Icon, label, onClick, danger = false }: { icon: any, label: string, onClick: () => void, danger?: boolean }) => (
        <button
            ...
            className={`w-full text-left px-3 py-2 text-sm font-medium flex items-center gap-2 transition-colors rounded-lg ${danger
                                    ...
                                    {isMenuOpen && createPortal(
                                        <>
                                            ...
                                            <div
                                                ...
                                                className={`fixed z-[9999] w-56 rounded-xl shadow-xl border p-1 animate-in fade-in zoom-in-95 duration-100 origin-top-right ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
                                                    }`}
                                        ...
                                        <div key={idx} className="p-4 space-y-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                                ...
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className={`font-mono font-bold ${isPerfect ? 'text-emerald-500' :
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        className={`px-4 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all border ${activeMenuId === 'modal'
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    {!selectedOrder.isArchived && !isOrderComplete(selectedOrder) && selectedOrder.status !== 'Storniert' && (
                                                        <button
                                                            ...
                                                            className={`w-full px-4 py-3 rounded-lg text-left flex items-center gap-3 transition-colors ${isDark ? 'hover:bg-blue-500/10 text-blue-400' : 'hover:bg-blue-50 text-blue-700'
                                                                }`}
                                                        >
                                                            <Pencil size={18} />
                                                    ...
                                                    {!selectedOrder.isArchived && !isOrderComplete(selectedOrder) && selectedOrder.status !== 'Storniert' && selectedOrder.items.reduce((s, i) => s + i.quantityReceived, 0) === 0 && (
                                                        <button
                                                            ...
                                                            className={`w-full px-4 py-3 rounded-lg text-left flex items-center gap-3 transition-colors ${isDark ? 'hover:bg-red-500/10 text-red-400' : 'hover:bg-red-50 text-red-600'
                                                                }`}
                                                        >
                                                            <Ban size={18} />
                                                            <div className="flex-1">
